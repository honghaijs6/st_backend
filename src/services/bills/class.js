...
class iRoute extends Service {
    ...
    async create(data,params){
        ...
        if(data_out.name==='success'){
          ...
          data.code = await this.Model._createCode(data.type);
          ...
          // CHECK IS FINISH BILL YET?
          const resCheckFinishBill =  await this.Model.isFinishBill(data.ref_code) ;
          data_out.data.is_finish_bill = resCheckFinishBill.message ;
    ...
    async cancel(data,params){
      ...
      if(data.id !== undefined && data.delete_reason_id	!== undefined){
        ...
        // CHECK IS FINISH BILL YET?
        const resCheckFinishBill =  await this.Model.isFinishBill(data.ref_code) ;
        ret.data.is_finish_bill = resCheckFinishBill.message ;
    ...
    async update(id,data,params){
      ...
      if(params.isMethod){
        ...
        if(ret.name==='success'){
          ...
          ret.data = await this.Model.getInfo(data.id);
